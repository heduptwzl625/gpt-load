...
function loadGroupData() {
  ...
  Object.assign(formData, {
    ...
    allowed_user_agents: (props.group.allowed_user_agents || []).join("\n"),
  });
}
        ...
        <div class="form-section" style="margin-top: 10px">
          <n-collapse>
            <n-collapse-item name="advanced">
              ...
              <div class="config-section">
                <n-form-item path="allowed_user_agents">
                  <template #label>
                    <div class="form-label-with-tooltip">
                      {{ t("keys.allowedUserAgents") }}
                      <n-tooltip trigger="hover" placement="top">
                        <template #trigger>
                          <n-icon :component="HelpCircleOutline" class="help-icon config-help" />
                        </template>
                        {{ t("keys.allowedUserAgentsTooltip") }}
                      </n-tooltip>
                    </div>
                  </template>
                  <n-input
                    v-model:value="formData.allowed_user_agents"
                    type="textarea"
                    :placeholder="t('keys.allowedUserAgentsPlaceholder')"
                    :rows="3"
                  />
                </n-form-item>
              </div>
